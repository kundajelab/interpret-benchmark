...
def compute_imp_scores(options):
    ...
    positives_data_loader =\
        SingleStreamSeqOnly(
            batch_size=50, bed_source="positives_not_in_small_valid_labels.gz",
    ...
    for model_id in options.model_ids:
        ...
        for method_name in [
            ...
            scores_flatref = np.array(
             scoring_func(input_data_list=[positives_X_subset],
            ...
            scores_avgposref = np.array(
                scoring_func(
                 input_data_list=[positives_X_subset],
                 input_references_list=[
                  np.array([avgpos_gcref[0] for x in positives_X_subset])],
                 task_idx=0, batch_size=10, progress_update=1000))
